...
from n_body_gravity.model import SpacetimeTransformer, EGNNSchnet, EGNNvelSchnet
import os
...
parser = argparse.ArgumentParser(
    description="Spatiotemporal Transformer Baseline Trajectory Prediction Example"
)
...
parser.add_argument(
    "--exp_name",
    type=str,
    default="spatiotemporal_transformer_baseline",
...
def create_summary_writer(
    ...
    log_dir = f"./runs/{dt}_transformer_baseline_optuna_{optuna}_lr_{lr}_bsz_{batch_size}_wd_{weight_decay}_n_egnn_l_{n_egnn_layers}_n_stacking_l_{n_stacking_layers}_hnf_{hidden_nf}_satt_{spatial_attention}_tatt_{temporal_attention}_pe_{positional_encoding}_ca_{causal_attention}_dropout_{dropout}_kappa_{kappa}_eq_{equivariant}_adj_{adjacency}_rec_{recurrent}_smt_{spatial_model_type}/"
...
def objective(trial):
    ...
    spatial_model_type = trial.suggest_categorical("spatial_model_type", ["egnn", "egnnvel", "egnnschnet", "egnnvelschnet"])
    ...
    use_velocity = False if spatial_model_type == "egnn" else True
    ...
    for epoch in range(0, args.epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(
                model.state_dict(),
                f"best_models/transformer_baseline_optuna_{True}_lr_{lr}_bsz_{batch_size}_wd_{weight_decay}_n_egnn_l_{n_egnn_layers}_n_stacking_l_{n_stacking_layers}_hnf_{hidden_nf}_satt_{spatial_attention}_tatt_{temporal_attention}_pe_{positional_encoding}_ca_{causal_attention}_dropout_{dropout}_kappa_{kappa}_eq_{equivariant}_adj_{adjacency}_rec_{recurrent}_{spatial_model_type}.pt",
            )
...
def main_optuna():
    ...
    file_name = f"optuna_study_transformer_baseline_equivariant_{trial.params['equivariant']}.txt"
...
def main():
    ...
    lr = args.lr
    spatial_model_type = args.spatial_model_type
    ...
    for epoch in range(0, args.epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(
                model.state_dict(),
                f"best_models/transformer_baseline_optuna_{False}_lr_{lr}_bsz_{batch_size}_wd_{weight_decay}_n_egnn_l_{n_egnn_layers}_n_stacking_l_{n_stacking_layers}_hnf_{hidden_nf}_satt_{spatial_attention}_tatt_{temporal_attention}_pe_{positional_encoding}_ca_{causal_attention}_dropout_{dropout}_kappa_{kappa}_eq_{equivariant}_adj_{adjacency}_rec_{recurrent}_{args.dataset}_{spatial_model_type}.pt",
            )
...
if __name__ == "__main__":
    if args.optuna:
        ...
        file_name = f"transformer_baseline_equivariant_{args.equivariant}_adjacency_{args.adjacency}_satt_{args.spatial_attention}_tatt_{args.temporal_attention}_{args.dataset}.txt"
        with open(file_name, "w") as file:
            file.write(text_out)
