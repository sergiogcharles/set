...
class E_GCL(nn.Module):
    ...
    def edge_model(self, source, target, radial, edge_attr):
        if edge_attr is None:  # Unused.
            ...
            # print(f'source: {source.shape} {target.shape} {radial.shape} {edge_attr.shape}')
            # breakpoint()
...
class E_GCL_vel(E_GCL):
    ...
    def forward(self, h, edge_index, coord, vel, edge_attr=None, node_attr=None):
        ...
        # print(f"h: {h.shape}, row: {row.shape}")
        # breakpoint()

        edge_feat = self.edge_model(h[row], h[col], radial, edge_attr)
        vel = self.coord_model(coord, edge_index, coord_diff, edge_feat) + (
            self.coord_mlp_vel(h) * vel
        )
        # coord += vel
        coord = coord + vel
