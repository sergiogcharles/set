...
def train(model, optimizer, epoch, loader, backprop=True):
    ...
    for batch_idx, data in enumerate(loader):
        ...
        if args.model == "gnn":
            ...
            nodes = torch.ones(loc.size(0), 1).to(
                device
            )  # all input nodes are set to 1
            rows, cols = edges
        ...
        if batch_idx % args.log_interval == 0 and (
            args.model == "se3_transformer" or args.model == "tfn"
        ):
            print(
...
def main_sweep():
    ...
    for epochs, tr_samples in zip(n_epochs, training_samples):
        ...
        print(results)
        print(
            "Results for %d epochs and %d # training samples \n" % (epochs, tr_samples)
        )
