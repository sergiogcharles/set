...
parser = argparse.ArgumentParser(
    description="Spatiotemporal Attention Baseline Trajectory Prediction Example"
)
...
parser.add_argument(
    "--exp_name",
    type=str,
    default="spatiotemporal_attention_baseline",
...
parser.add_argument("--tensorboard", type=str_to_bool, default=False, help="Uses tensorboard")
...
class EarlyStopper:
    def __init__(self, patience=1, min_delta=0, plateau_patience=5, plateau_threshold=1e-4):
...
def create_summary_writer(
    ...
    log_dir = f"./runs/{dt}_attention_baseline_optuna_{optuna}_lr_{lr}_wd_{weight_decay}_bsz_{batch_size}_hnf_{hidden_nf}_satt_{spatial_attention}_tatt_{temporal_attention}_pe_{positional_encoding}_ca_{causal_attention}_dropout_{dropout}_kappa_{kappa}_eq_{equivariant}_n_egnn_l_{n_egnn_layers}_rec_{recurrent}_adj_{adjacency}/"
...
def objective(trial):
    ...
    n_egnn_layers = trial.suggest_categorical("n_egnn_layers", [2, 3, 5])
    ...
    for epoch in range(0, args.epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(
                model.state_dict(),
                f"best_models/attention_baseline_optuna_{True}_lr_{lr}_wd_{weight_decay}_bsz_{batch_size}_hnf_{hidden_nf}_satt_{spatial_attention}_tatt_{temporal_attention}_pe_{positional_encoding}_ca_{causal_attention}_dropout_{dropout}_kappa_{kappa}_eq_{equivariant}_n_egnn_l_{n_egnn_layers}_rec_{recurrent}_adj_{adjacency}.pt",
            )
...
def main_optuna():
    ...
    file_name = (
        f"optuna_study_attention_baseline_equivariant_{trial.params['equivariant']}.txt"
    )
...
def train(model, optimizer, epoch, loader):
    ...
    for batch_idx, data in enumerate(loader):
        ...
        ) = model(h, loc.detach(), edges, vel.detach(), edge_attr)

        # (B * N * L, n) -> (B, N, L, n)
        loc_spatiotemporal = loc_spatiotemporal.reshape(B, N, L, n)
        # Take a mean across the time dimension: (B, N, L, n) -> (B, N, n)
        loc_spatiotemporal = loc_spatiotemporal.mean(dim=-2)
        ...
        loss = loss_mse(loc_spatiotemporal, loc_end) + loss_mse(vel_spatiotemporal, vel_end)
...
if __name__ == "__main__":
    best_train_loss, best_val_loss, best_test_loss, best_epoch, total_params = main()
    # Record best metrics
    ...
    file_name = f"attention_baseline_equivariant_{args.equivariant}_adjacency_{args.adjacency}_satt_{args.spatial_attention}_tatt_{args.temporal_attention}.txt"
    with open(file_name, "w") as file:
        file.write(text_out)
