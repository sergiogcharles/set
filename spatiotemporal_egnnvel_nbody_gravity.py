...
from n_body_gravity.model import (
    EGNNvelBaseline,
)
import os
...
parser = argparse.ArgumentParser(
    description="Spatiotemporal EGNNvel Baseline Trajectory Prediction Example"
)
...
def create_summary_writer(
    ...
    log_dir = f"./runs/{dt}_egnnvel_baseline_optuna_{optuna}_lr_{lr}_bsz_{batch_size}_wd_{weight_decay}_n_egnn_l_{n_egnn_layers}_hnf_{hidden_nf}_rec_{recurrent}_natoms_{num_atoms}/"
...
def objective(trial):
    ...
    hidden_nf = trial.suggest_categorical("hidden_nf", [32, 128])
    ...
    for epoch in range(0, args.epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(
                model.state_dict(),
                f"best_models/egnnvel_baseline_optuna_{True}_lr_{lr}_bsz_{batch_size}_wd_{weight_decay}_n_egnn_l_{n_egnn_layers}_hnf_{hidden_nf}_rec_{recurrent}_natoms_{num_atoms}.pt",
            )
...
def main_optuna():
    ...
    file_name = (
        f"optuna_study_egnnvel_baseline_equivariant_{trial.params['equivariant']}.txt"
    )
...
if __name__ == "__main__":
    if args.optuna:
        ...
        file_name = f"results/egnnvel_baseline_equivariant_{args.equivariant}_adjacency_{args.adjacency}_satt_{args.spatial_attention}_tatt_{args.temporal_attention}_{args.dataset}.txt"
        with open(file_name, "w") as file:
            file.write(text_out)